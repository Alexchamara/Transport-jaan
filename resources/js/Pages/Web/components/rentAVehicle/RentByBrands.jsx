  ...
  return (
    <div className="w-full py-12 mt-10">
      <div className="container mx-auto px-20">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-10 cursor-pointer">
          {brands.map((brand) => (
            <div
              key={brand.name}
              onClick={() => handleBrandClick(brand.name)}
              className="bg-[#EAEAE9] border-[1px] border-[#0955AC] p-4 rounded-[8px] shadow flex flex-col items-center justify-center h-[137px] hover:bg-[#f5f5f5] transition-colors duration-300"
            >
              <img
                src={brand.logo}
                alt={`${brand.name} Logo`}
                className="object-contain mb-5 w-[42px] h-[42px]"
              />
              <p className="figtree text-[#0F0F0F] text-[12px] md:text-[16px] font-[500] text-center">
                {brand.name}
              </p>
            </div>
          ))}
