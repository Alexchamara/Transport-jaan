  ...
  if (isMobile) {
    return (
      <div className="flex flex-col gap-4 p-4">
        {days.map((day, dayIdx) => (
          <div key={dayIdx} className="bg-white rounded-lg p-4 shadow border">
            <h2 className="text-lg font-bold mb-4">{day.label} {day.date}</h2>
            <div className="space-y-2">
              {times.map(time => {
                const cellEvents = events.filter(e => e.day === dayIdx && e.time === time);
                  ...
                  <div key={time} className="border-l-4 border-blue-500 pl-4 py-2">
                    ...
                    {cellEvents.map((event, idx) => {
                      const eventIdx = events.findIndex(
                        (e) => e.day === dayIdx && e.time === time && e.title === event.title && e.person === event.person && e.status === event.status
                      );
                      return (
                        <div
                          key={idx}
                          className={`p-3 rounded-lg mb-2 ${event.status === "done" ? "bg-blue-50 border border-blue-200" : "bg-red-50 border border-red-200"} cursor-pointer`}
                          onClick={() => handleEditEvent(eventIdx)}
                        >
                          <div className="font-medium text-sm mb-1">{event.title}</div>
                          <div className="flex items-center">
                            <img src={proPicTwo} alt="avatar" className="w-6 h-6 rounded-full mr-2" />
                            <span className="text-xs text-gray-600">{event.person}</span>
                          </div>
                        </div>
                      );
                    })}
              ...
              {events.filter(e => e.day === dayIdx).length === 0 && (
                <div className="text-center text-gray-500 py-4">No events</div>
              )}
            </div>
          </div>
        ))}
