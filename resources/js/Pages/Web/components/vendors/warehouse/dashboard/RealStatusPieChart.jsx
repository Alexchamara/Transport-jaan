...
const defaultData = [
  { name: "Active", value: 46, color: "#3DD0FF", arrow: up, arrowAlt: "up", change: "up" },
  { name: "Pending", value: 27, color: "#0955AC", arrow: down, arrowAlt: "down", change: "down" },
  { name: "Inactive", value: 27, color: "#C4C4C4", arrow: up, arrowAlt: "up", change: "up" },
];
...
const RealStatusPieChart = ({ data = [] }) => {
  const chartData = data.length > 0 ? data.map(item => ({
    ...item,
    arrow: item.change === 'up' ? up : down,
    arrowAlt: item.change || 'up'
  })) : defaultData;
