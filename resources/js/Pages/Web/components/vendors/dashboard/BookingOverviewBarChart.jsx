  ...
  return (
    <div className="w-[600px] h-auto flex flex-col items-stretch relative">
      <div className="relative w-full" style={{ height: `${chartHeight}px` }}>
          ...
          {[1000, 750, 500, 250, 0].map((v) => {
            ...
            return (
              <div key={v} className="w-full absolute flex items-center" style={{ bottom: `${percentFromBottom}%` }}>
                <span className="text-[14px] text-gray-400 absolute -left-12 -top-7 w-8 text-left" style={{ transform: "translateY(50%)" }}>
                  {v === 1000 ? "1K" : v}
                </span>
        ...
        {/* bars */}
        <div className="flex flex-row items-end w-full h-full z-10 relative" style={{ height: `${chartHeight}px` }}>
          {bookingData.map((d, i) => {
              ...
              <div key={d.name ?? i} className="flex flex-col items-center flex-1 relative group">
                ...
                {(hovered === i || (hovered === null && i === maxIndex)) && (
                  <div className="absolute -top-20 left-1/2 -translate-x-1/2 bg-[#D8E4F2] text-black px-6 py-2 rounded-lg shadow text-center z-20">
                    <div className="font-[600] text-[14px]">{d.name ?? "-"}</div>
                    <div className="text-[16px] font-[700]">{val}</div>
                  </div>
                )}
              </div>
            );
      ...
      {/* labels */}
      <div className="flex flex-row items-end w-full z-10 relative mt-2">
        {bookingData.map((d, i) => (
          <div key={d.name ?? i} className="flex-1 flex justify-center" style={{ minWidth: "36px" }}>
