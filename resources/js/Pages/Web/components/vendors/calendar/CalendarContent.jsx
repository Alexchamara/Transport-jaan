...
const CalendarContent = ({ events: initialEvents, clients, currentMonth: initialMonth, currentYear: initialYear, selectedUserId }) => {
  ...
  // Convert events to the format expected by CalendarGrid
  const processedEvents = initialEvents?.map((event) => {
    const pickupDate = new Date(event.pickup_at);
    const dayOfWeek = (pickupDate.getDay() + 6) % 7; // Convert to Monday = 0
            ...
            <>
              <div className="flex flex-row gap-2 justify-center items-center w-full h-auto bg-[#E5E5E5] rounded-[10px] px-5 py-5">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex flex-row gap-10 text-[16px] font-[500]">
                    ...
                    <div className="flex flex-col gap-3">
                      <h1>{new Date(selectedBooking.pickup_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</h1>
                      <h1>{new Date(selectedBooking.dropoff_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</h1>
                    </div>
                  </div>
              ...
              <div className="flex flex-row gap-2 justify-center items-center w-full h-auto bg-[#E5E5E5] rounded-[10px] px-5 py-5">
                ...
                <div className="flex flex-col gap-2">
                  <h1 className="text-[18px] font-[700]">{selectedBooking.vehicle?.name || selectedBooking.title}</h1>
                  <div className="flex flex-row gap-10 text-[16px] font-[500]">
        ...
        >
          ...
          <div className="py-5 flex flex-col justify-center items-center gap-5">
            {initialEvents?.slice(0, 4).map((event, idx) => (
              <div key={idx} className="w-[286px] h-[68px] bg-[#D8E4F2] rounded-[10px] flex flex-row justify-center items-center gap-5 px-3 py-2">
                ...
                <h1 className="text-[14px] font-[500] w-[199px]">
                  {event.title} pickup at {event.pickup_time} on {event.pickup_date}
                </h1>
              </div>
            ))}
      ...
      >
        <div className="px-20 flex flex-row items-center justify-between">
          <div className="flex flex-row justify-center items-center gap-6">
            ...
            <div className="flex flex-row justify-center items-center gap-2">
              <div className="size-[35px] bg-[#F3F3F3] rounded-[6px] flex justify-center items-center cursor-pointer hover:bg-[#e0e0e0]" onClick={handlePrevMonth}>
                <img src={leftArrow} alt="previous" />
              </div>
              <div className="size-[35px] bg-[#F3F3F3] rounded-[6px] flex justify-center items-center cursor-pointer hover:bg-[#e0e0e0]" onClick={handleNextMonth}>
                <img src={leftArrow} className="rotate-180" alt="next" />
              </div>
            </div>
            <h1 className="text-[18px] font-[700]">
        ...
        <div className="flex flex-row gap-10 justify-start items-center px-20 py-5">
          <div className="flex flex-row justify-start items-center gap-5">
            ...
            <h1 className="text-[#00000080] font-[600] text-[16px]">Done</h1>
          </div>
          <div className="flex flex-row justify-start items-center gap-5">
            <div className="size-[16px] bg-[#FFDBDF] rounded-[4px]" />
            <h1 className="text-[#00000080] font-[600] text-[16px]">Cancelled</h1>
          </div>
        </div>
        ...
        <div className="grid grid-cols-8 border-t border-l border-[#00000026]">
          <CalendarGrid
            ...
            onEventClick={(event) => setSelectedBooking(event.fullData)}
          />
        </div>
      </div>
    </div>
