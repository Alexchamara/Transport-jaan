...
const Land = ({ statusFilter, approvalFilter }) => {
    const [vehicles, setVehicles] = useState([
        { id: 1, name: "Wagon R", model: "Suzuki", number: "CBD-8656", price: 200, status: "Active", approval: "Approved" },
        { id: 2, name: "Maruti", model: "Suzuki", number: "CAR-2556", price: 180, status: "Inactive", approval: "Pending" },
        { id: 3, name: "X1", model: "BMW", number: "CBJ-8777", price: 600, status: "Inactive", approval: "Canceled" },
        { id: 4, name: "Civic", model: "Honda", number: "XYZ-1234", price: 250, status: "Active", approval: "Approved" },
        { id: 5, name: "Fortuner", model: "Toyota", number: "ABC-5678", price: 500, status: "Active", approval: "Pending" },
        { id: 6, name: "Polo", model: "Volkswagen", number: "DEF-9012", price: 220, status: "Inactive", approval: "Approved" },
        { id: 7, name: "Creta", model: "Hyundai", number: "GHI-3456", price: 350, status: "Active", approval: "Approved" },
        { id: 8, name: "Swift", model: "Maruti", number: "JKL-7890", price: 190, status: "Inactive", approval: "Canceled" },
        { id: 9, name: "Innova", model: "Toyota", number: "MNO-2345", price: 450, status: "Active", approval: "Pending" },
        { id: 10, name: "A4", model: "Audi", number: "PQR-6789", price: 700, status: "Inactive", approval: "Approved" },
    ]);
    ...
    // Filter vehicles based on status and approval
    const filteredVehicles = vehicles.filter((vehicle) => {
        const matchesStatus =
            statusFilter === "" || statusFilter === "all" || vehicle.status === statusFilter;
        const matchesApproval =
            approvalFilter === "" || approvalFilter === "all" || vehicle.approval === approvalFilter;
    ...
    const getApprovalStyles = (approval) => {
        switch (approval) {
            case "Approved":
                return { border: "border-[#05C16880]", bg: "bg-[#05C16833]", dot: "bg-[#14CA74]", text: "text-green-400" };
            case "Pending":
                return { border: "border-[#FFB01633]", bg: "bg-[#FFB01633]", dot: "bg-[#FDB52A]", text: "text-[#FDB52A]" };
            case "Canceled":
                return { border: "border-[#FF5A6533]", bg: "bg-[#FF5A6533]", dot: "bg-[#FF5A65]", text: "text-[#FF5A65]" };
        ...
        <div className="poppins flex flex-col items-center min-h-screen w-full gap-1">
            <div className="w-[962px] flex flex-row justify-between items-center text-white text-[16px] font-500 pt-6">
                ...
                <h1 className="text-[#0955AC] text-[14px] font-400">
                    1 - {filteredVehicles.length} <span className="text-[#AEB9E1]">of {vehicles.length}</span>
                </h1>
            </div>
            ...
            <div className="flex flex-row justify-center items-center w-full h-[61px]">
                <div className="flex flex-row justify-start items-start w-full px-[35px]">
                    <div className="flex flex-row justify-start items-center gap-4 w-[180px]">
                        <h1 className="text-white text-[10px] font-400">Vehicle name</h1>
                    </div>
                    <div>
                        <h1 className="text-white text-[10px] font-400 w-[150px]">Model</h1>
                    </div>
                    <div>
                        <h1 className="text-white text-[10px] font-400 w-[150px]">Number</h1>
                    </div>
                    <div>
                        <h1 className="text-white text-[10px] font-400 w-[150px]">Rental price / day</h1>
                    </div>
                    <div>
            ...
            {filteredVehicles.map((vehicle, index) => (
                <div key={index} className="flex flex-row justify-center items-center w-full h-[61px]">
                    <div className="flex flex-row justify-start items-start w-full px-[35px]">
                        <div>
                            <h1 className="text-[#AEB9E1] text-[10px] font-400 w-[180px]">{vehicle.name}</h1>
                        </div>
                        <div>
                            <h1 className="text-[#AEB9E1] text-[10px] font-400 w-[150px]">{vehicle.model}</h1>
                        </div>
                        <div>
                            <h1 className="text-[#AEB9E1] text-[10px] font-400 w-[150px]">{vehicle.number}</h1>
                        </div>
                        <div>
                            <h1 className="text-[#AEB9E1] text-[10px] font-400 w-[150px]">${vehicle.price}</h1>
                        </div>
                        <div className="w-[150px]">
                            <div className={`flex flex-row justify-center items-center gap-1 border ${getStatusStyles(vehicle.status).border} ${getStatusStyles(vehicle.status).bg} px-[6px] py-[2px] rounded-[5px] w-[70px]`}>
                                <div className={`w-1 h-1 rounded-full ${getStatusStyles(vehicle.status).dot}`} />
                                <h1 className={`${getStatusStyles(vehicle.status).text} text-[10px] font-500 flex flex-row justify-center items-center`}>
                                    {vehicle.status}
                                </h1>
                            </div>
                        </div>
                        <div className="w-[150px]">
                            <div className={`flex flex-row justify-center items-center gap-1 border ${getApprovalStyles(vehicle.approval).border} ${getApprovalStyles(vehicle.approval).bg} px-[6px] py-[2px] rounded-[5px] w-[70px]`}>
                                <div className={`w-1 h-1 rounded-full ${getApprovalStyles(vehicle.approval).dot}`} />
                                <h1 className={`${getApprovalStyles(vehicle.approval).text} text-[10px] font-500 flex flex-row justify-center items-center`}>
                                    {vehicle.approval}
                                </h1>
                            </div>
                        </div>
