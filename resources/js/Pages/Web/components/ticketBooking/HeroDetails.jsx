  ...
  return (
    <section className="mx-auto w-full max-w-6xl px-6 py-8">
      ...
      <div className="sticky top-0 z-10 -mx-6 mb-4 border-b bg-white/80 px-6 py-5 backdrop-blur">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex flex-wrap gap-4">
            {['Fare', 'Departure', 'Arrival', 'Seats Availability', 'Name'].map((f) => (
              <button
                key={f}
      ...
      <div className="space-y-6">
        {trips.map((trip) => (
          <div key={trip.id} className="rounded-xl border border-gray-300 bg-white p-8 shadow-md transition hover:shadow-xl">
              ...
              <div className="col-span-12 sm:col-span-5">
                <div className="flex items-center justify-between gap-6">
                  <div>
                    ...
                    <h3 className="text-xl font-extrabold text-gray-900">{trip.name}</h3>
                    <p className="mt-2 text-lg text-gray-600">{trip.route}</p>
                  </div>

                  <div className="hidden gap-3 sm:flex">
                    {['AC', 'W', 'TV', 'USB', 'CCTV', 'WIFI'].map((a) => (
                      <span
                        key={a}
                        title={a}
              ...
              <div className="col-span-12 sm:col-span-4">
                <div className="flex items-center justify-center sm:justify-start">
                  <div className="text-center sm:text-left">
                    <div className="text-lg font-bold text-gray-900">{trip.depart}</div>
                    <div className="text-base text-gray-600">{trip.days}</div>
                  </div>
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-lg font-bold text-gray-800">{trip.duration}</div>
                  </div>

                  <span className="mx-4 inline-block h-3 w-3 rounded-full bg-gray-400 align-middle" />

                  <div className="text-center sm:text-right">
                    <div className="text-lg font-bold text-gray-900">{trip.arrive}</div>
                    <div className="text-base text-gray-600">{trip.days}</div>
                  </div>
                </div>
                <div className="mt-5 flex flex-wrap gap-6 text-lg text-[#0955AC]">
                  <button className="hover:underline">View bus stops & times</button>
                  <button className="hover:underline text-gray-600">View cancellation policy</button>
                </div>
              </div>
              ...
              <div className="col-span-12 sm:col-span-3">
                <div className="flex flex-col items-end gap-4 sm:items-end">
                  <div className="text-right">
                    <div className="text-2xl font-extrabold text-gray-900">LKR {trip.price.toLocaleString()}</div>
                    <div className="text-lg text-gray-600">Available seats: 36/45</div>
                  </div>
                  <button
