...
/**
 * BusTicketBookingPreview
 * - Seat selection + passenger form (UI modeled after the screenshot)
 * - Uses brand color #0955AC
 * - No backend wiring yet; on submit logs the collected data
 */
...
const BusTicketBookingPreview = () => {
  ...
  const toggleSeat = (seatId, status) => {
    if (status === STATUS.BOOKED || status === STATUS.NOT_PROVIDED || status === STATUS.IN_PROGRESS) return;
    setSelected((prev) =>
        ...
        <div className="lg:col-span-2">
          <div className="flex justify-center">
            <div className="inline-block">
              {/* Front label */}
              <div className="mx-auto mb-3 w-[220px] rounded-md bg-gray-100 py-2 text-center text-gray-700 font-semibold">
                Front
              </div>
              ...
              <div className="flex">
                <div className="mr-4 flex flex-col items-end pr-2">
                  {Array.from({ length: ROWS }).map((_, i) => (
                    <div key={i} className="h-12 leading-[48px] text-gray-600 font-medium">
                      {i + 1}
                    </div>
                  ))}
                </div>
              ...
              <div className="mt-3 text-gray-600 font-medium text-right pr-2">13</div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="rounded-md border border-gray-200 bg-gray-50">
            <div className="px-5 py-4 border-b border-gray-200">
              <h2 className="text-xl font-semibold text-gray-800">Seat Details</h2>
            </div>

            <form onSubmit={onSubmit} className="px-5 py-4 space-y-4">
              {/* Seats */}
              <div className="flex items-center justify-between">
                <span className="text-gray-700 font-semibold">Seats</span>
                {selected.length === 0 ? (
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-gray-900">{total.toLocaleString()} LKR</span>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-md border border-gray-300 px-3 py-2 focus:border-[#0955AC] focus:ring-[#0955AC]"
                />
              </div>
              ...
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => setReuseCredits(e.target.checked)}
                  className="h-4 w-4 text-[#0955AC] focus:ring-[#0955AC]"
                />
                <span className="text-sm text-gray-700">Reuse Credits</span>
              </label>

              <button
                type="submit"
                disabled={!canContinue}
                className={`mt-2 w-full rounded-md px-4 py-3 text-white font-semibold ${
                  canContinue
...
const SeatButton = ({ seat, selected, onClick }) => {
  const base = "h-12 w-12 rounded-md text-sm font-semibold flex items-center justify-center";
  let cls = "bg-gray-200 text-gray-700";
  if (seat.status === STATUS.AVAILABLE) cls = "bg-gray-200 text-gray-800 hover:bg-[#62B36F]/70";
  if (seat.status === STATUS.BOOKED) cls = "bg-[#C7C7C7] text-gray-600 cursor-not-allowed";
  if (seat.status === STATUS.IN_PROGRESS) cls = "bg-black text-white cursor-not-allowed";
  if (seat.status === STATUS.NOT_PROVIDED) cls = "bg-[#F5F5DC] text-gray-800 cursor-not-allowed";
