...
const VehicleReports = ({ bookings = [], stats = {} }) => {
  ...
  const filteredBookings = useMemo(() => {
    ...
    return bookings.filter((booking) => {
      ...
      const matchesType = typeFilter === 'All' || booking.vehicle_type === typeFilter;
  ...
  const handleExport = (format) => {
    const headers = ['Booking Ref', 'Vehicle Type', 'Status', 'Amount', 'Date'];
    ...
    if (format === 'csv') {
      ...
      link.href = url;
      link.download = 'vehicle-reports.csv';
      link.click();
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'vehicle-reports.xls';
      link.click();
      URL.revokeObjectURL(url);
    } else if (format === 'pdf') {
      const doc = new jsPDF();
      doc.setFontSize(18);
      doc.text('Vehicle Bookings Report', 14, 20);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-[420px] p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-gray-900">Export Vehicle Reports</h2>
              <button 
