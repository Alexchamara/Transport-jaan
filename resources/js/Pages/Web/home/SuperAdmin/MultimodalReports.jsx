...
const MultimodalReports = ({ bookings = [], stats = {} }) => {
  ...
  useEffect(() => {
    fetch('/SuperAdmin/reports/filter-options')
      .then(res => res.json())
      .then(data => {
        setStatusOptions(data.multimodalStatuses || []);
        setDateOptions(['All', 'Last 7 Days', 'Last 30 Days', 'This Year']);
      })
      .catch(err => console.error('Failed to fetch filter options:', err));
  }, []);

  // Extract journey counts from bookings data
  useEffect(() => {
    const journeys = Array.from(new Set(bookings.map((booking) => booking.journey_count).filter(Boolean)));
  ...
  const filteredBookings = useMemo(() => {
    ...
    return bookings.filter((booking) => {
      ...
      const matchesJourney =
        journeyFilter === 'All' || String(booking.journey_count) === journeyFilter;
      const matchesDate = matchesDateFilter(booking.created_at);

      return matchesSearch && matchesStatus && matchesJourney && matchesDate;
    });
  }, [bookings, searchTerm, statusFilter, journeyFilter, dateFilter]);
  ...
  const handleExport = (format) => {
    const headers = ['Booking Ref', 'Passenger', 'Journeys', 'Status', 'Amount', 'Date'];
    ...
    if (format === 'csv') {
      ...
      link.href = url;
      link.download = 'multimodal-reports.csv';
      link.click();
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'multimodal-reports.xls';
      link.click();
      URL.revokeObjectURL(url);
    } else if (format === 'pdf') {
      const doc = new jsPDF();
      doc.setFontSize(18);
      doc.text('Multimodal Bookings Report', 14, 20);
      ...
      doc.save('multimodal-reports.pdf');
    }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-[420px] p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-gray-900">Export Multimodal Reports</h2>
              <button 
