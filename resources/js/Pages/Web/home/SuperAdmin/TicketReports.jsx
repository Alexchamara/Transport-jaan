...
const TicketReports = ({ bookings = [], stats = {} }) => {
  ...
  // Extract ticket types from bookings data
  useEffect(() => {
    const ticketTypes = Array.from(new Set(bookings.map((booking) => booking.type).filter(Boolean)));
    setTypeOptions(ticketTypes);
  }, [bookings]);
  ...
  const handleExport = (format) => {
    const headers = ['Type', 'Booking Ref', 'Customer', 'Status', 'Date'];
    ...
    if (format === 'csv') {
      ...
      link.href = url;
      link.download = 'ticket-reports.csv';
      link.click();
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'ticket-reports.xls';
      link.click();
      URL.revokeObjectURL(url);
    } else if (format === 'pdf') {
      const doc = new jsPDF();
      doc.setFontSize(18);
      doc.text('Ticket Bookings Report', 14, 20);
    ...
    <div className='flex flex-row bg-[#081028] min-h-screen sm:flex-col md:flex-row lg:flex-row poppins'>
      ...
      <div className='flex-1 p-8'>
        <div className='max-w-7xl mx-auto'>
          <h1 className='text-white text-3xl font-bold mb-6'>Ticket Bookings Report</h1>
          <p className='text-gray-400 mb-8'>Comprehensive report of all ticket bookings.</p>
          ...
          <div className='grid grid-cols-1 md:grid-cols-4 gap-6 mb-8'>
            ...
            <div className='bg-[#181A2A] rounded-lg p-6 border border-gray-700'>
              <h3 className='text-gray-400 text-sm mb-2'>Flight Tickets</h3>
              <p className='text-white text-2xl font-bold'>{stats.flightBookings || 0}</p>
            </div>
            <div className='bg-[#181A2A] rounded-lg p-6 border border-gray-700'>
              <h3 className='text-gray-400 text-sm mb-2'>Bus Tickets</h3>
              <p className='text-white text-2xl font-bold'>{stats.busBookings || 0}</p>
            </div>
            <div className='bg-[#181A2A] rounded-lg p-6 border border-gray-700'>
              <h3 className='text-gray-400 text-sm mb-2'>Train Tickets</h3>
              <p className='text-white text-2xl font-bold'>{stats.trainBookings || 0}</p>
            </div>
          </div>
                ...
                <table className='w-full text-left'>
                  <thead>
                    <tr className='border-b border-gray-700'>
                      <th className='text-gray-400 pb-3 px-2'>Type</th>
                      <th className='text-gray-400 pb-3 px-2'>Booking Ref</th>
                      <th className='text-gray-400 pb-3 px-2'>Customer</th>
                      ...
                      <tr key={index} className='border-b border-gray-800'>
                        <td className='text-white py-3 px-2'>
                          <span className={`px-2 py-1 rounded text-xs ${
                            booking.type === 'Flight' ? 'bg-blue-600 text-white' :
                            booking.type === 'Bus' ? 'bg-purple-600 text-white' :
                        ...
                        <td className='text-gray-300 py-3 px-2'>{booking.customer_name}</td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-[420px] p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-gray-900">Export Ticket Reports</h2>
              <button 
