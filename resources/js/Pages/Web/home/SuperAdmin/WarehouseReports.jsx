...
const WarehouseReports = ({ bookings = [], stats = {} }) => {
  ...
  const filteredBookings = useMemo(() => {
    ...
    return bookings.filter((booking) => {
      ...
      const matchesType = typeFilter === 'All' || booking.storage_type === typeFilter;
  ...
  const handleExport = (format) => {
    const headers = ['Booking Ref', 'Company', 'Storage Type', 'Status', 'Amount', 'Date'];
    const rows = filteredBookings.map((booking) => [
      booking.booking_reference,
      booking.company_name,
      booking.storage_type,
      booking.status,
      `LKR ${booking.final_amount.toLocaleString()}`,
    ...
    if (format === 'csv') {
      ...
      link.href = url;
      link.download = 'warehouse-reports.csv';
      link.click();
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'warehouse-reports.xls';
      link.click();
      URL.revokeObjectURL(url);
    } else if (format === 'pdf') {
      const doc = new jsPDF();
      doc.setFontSize(18);
      doc.text('Warehouse Bookings Report', 14, 20);
      ...
      doc.save('warehouse-reports.pdf');
    }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-[420px] p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-gray-900">Export Warehouse Reports</h2>
              <button 
