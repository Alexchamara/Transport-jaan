...
/** SideMenu: unchanged visually; links fixed to /vendors/... and
 *  Units kept active for add/edit routes. Accepts optional active prop. */
const SideMenu = ({ active }) => {
  ...
  const isDashboard = currentPath.startsWith('/vendors/dashboard');
  const isBookings  = currentPath.startsWith('/vendors/bookings');
  const isUnits     =
    active === 'units' ||
    currentPath === '/vendors/units' ||
    currentPath.startsWith('/vendors/units') ||
    currentPath.startsWith('/vendor/vehicles') ||
    currentPath.startsWith('/vendors/vehicles');

  const isCalendar  = currentPath.startsWith('/vendors/calendar');
  const isClients   = currentPath.startsWith('/vendors/clients');
  const isPayment   = currentPath.startsWith('/vendors/payment');
  const isExpenses  = currentPath.startsWith('/vendors/expenses');
  const isFinancial = isPayment || isExpenses;
  const isTracking  = currentPath.startsWith('/vendors/tracking');
  const isMessage   = currentPath.startsWith('/vendors/message');

  const itemCls = (active) =>
    `flex flex-row justify-start items-center gap-5 cursor-pointer w-full rounded-lg px-4 py-2 ${
      active ? "bg-[#0955AC29] text-[#000000] font-[700]" : "text-[#00000066]"
    }`;
...
/** Keep “Other” visible (requested), but we’ll map it to a DB-safe value on submit */
...
const AddUnit = () => {
  ...
  useEffect(() => {
    ...
    if (Array.isArray(vehicle.media)) {
      existingVehicleImgs = extractFromSpatieMedia(vehicle.media, [
        'images', 'vehicle_images', 'vehicles', 'vehicle-photos',
      ]);
    }
    ...
    if (Array.isArray(vehicle.media)) {
      existingInsuranceImgs = extractFromSpatieMedia(vehicle.media, [
        'insurance', 'insurance_docs', 'insurance_photos', 'vehicle-insurance',
      ]);
    }
    ...
    setInsuranceFiles([]); setInsurancePreviews([]);
  ...
  const validate = () => {
    const e = {};
    const must = [
      'category','model','manufacture','manufactureYear','registerYear','number','colour',
      'condition','ownershipType','passengerCapacity','rentalPricePerDay','deposit','advancePayment',
    ];
    ...
    if (form.category === 'Sea') {
      ['vessel_type','hull_material','length_m','beam_m','draft_m','engine_type','engine_power_hp','sea_fuel_type'].forEach((k) => {
        if (!String(form[k] || '').trim()) e[k] = 'This field is required.';
      });
    }
    return e;
  ...
  const handleSubmit = (e) => {
    ...
    /** ===== ENUM NORMALIZATION & GUARDS (fixes MySQL truncation) ===== */
    if (form.category === 'Land') {
      const body_type = toEnum(form.bodyType);
      const fuel_type = toEnum(form.fuelType);

      // normalize transmission; map "Other" to a DB-safe value (fallback: manual)
      const wanted = toEnum(form.transmissionType);
      const transmission_type = ALLOWED_TRANSMISSIONS.includes(wanted) ? wanted : 'manual';

      if (isGearsIrrelevant(transmission_type)) {
        data.set('gears', '');
      }

      // set both snake_case and camelCase to keep back-end happy
      data.set('bodyType', body_type);
      data.set('fuelType', fuel_type);
      data.set('transmissionType', transmission_type);

      data.set('body_type', body_type);
      data.set('fuel_type', fuel_type);
      data.set('transmission_type', transmission_type);

      if (form.fuelTankCapacity !== undefined) {
        data.set('fuel_tank_capacity_l', String(form.fuelTankCapacity));
      }
    }
    /** ===== END ===== */
  ...
  const handleAddAnother = () => {
    ...
    setExistingImages([]); setExistingInsurance([]);
  ...
  const addExtraFeature = () => {
    setForm((prev) => ({ ...prev, extraFeatures: [...(prev.extraFeatures || []), { name: '', price: '' }] }));
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      {/* Side menu with everything kept */}
      <SideMenu active="units" />

      <main className="flex-1">
        {/* Header text stays the same even when editing */}
              ...
              <section className="bg-[#FFFFFF] p-6 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <select id="ownershipType" name="ownershipType" className={selectClasses('ownershipType')} value={form.ownershipType} onChange={handleChange} {...req('Please select an ownership type.')}>
                          ...
                          {ownershipTypeOptions.map((c) => (<option key={c} value={c}>{c}</option>))}
                        </select>
                          ...
                          <div className="flex text-sm text-gray-600">
                            ...
                            >
                              ...
                              <input
                                ...
                                onChange={handleChange}
                                className="sr-only" disabled={!canAddMoreInsurance}
                              />
                            </label>
                    ...
                    {/* Current Insurance (below uploads) */}
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="space-y-2">
                      ...
                      <input
                        id="rentalPricePerDay" type="number" name="rentalPricePerDay" min="0" step="0.01"
                        className={inputClasses('rentalPricePerDay')} value={form.rentalPricePerDay}
                        onChange={handleChange} placeholder="0.00" {...req('Please enter daily rental price.')}
                      />
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        id="deposit" type="number" name="deposit" min="0" step="0.01"
                        className={inputClasses('deposit')} value={form.deposit}
                        onChange={handleChange} placeholder="0.00" {...req('Please enter the deposit amount.')}
                      />
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        id="advancePayment" type="number" name="advancePayment" min="0" step="0.01"
                        className={inputClasses('advancePayment')} value={form.advancePayment}
                        onChange={handleChange} placeholder="0.00" {...req('Please enter the advance payment.')}
                      />
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                      ...
                      <div className="md:col-span-2">
                        <input
                          type="number" min="0" step="0.01" name="gpsPrice" value={form.gpsPrice}
                          onChange={handleChange} placeholder="GPS price per day (0.00)" className={inputClasses('gpsPrice')}
                          disabled={!form.gps} {...(form.gps ? req('Enter GPS price.') : {})}
                        />
                    ...
                    {form.category === 'Land' && (
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                        ...
                        <div className="md:col-span-2">
                          <input
                            type="number" min="0" step="0.01" name="childSeatPrice" value={form.childSeatPrice}
                            onChange={handleChange} placeholder="Child seat price per day (0.00)" className={inputClasses('childSeatPrice')}
                            disabled={!form.childSeat} {...(form.childSeat ? req('Enter child seat price.') : {})}
                          />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                      ...
                      <div className="md:col-span-2">
                        <input
                          type="number" min="0" step="0.01" name="wifiPrice" value={form.wifiPrice}
                          onChange={handleChange} placeholder="Wi-Fi price per day (0.00)" className={inputClasses('wifiPrice')}
                          disabled={!form.wifi} {...(form.wifi ? req('Enter Wi-Fi price.') : {})}
                        />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                      ...
                      <div className="md:col-span-2">
                        <input
                          type="number" min="0" step="0.01" name="insuranceCoveragePrice" value={form.insuranceCoveragePrice}
                          onChange={handleChange} placeholder="Insurance coverage price per day (0.00)" className={inputClasses('insuranceCoveragePrice')}
                          disabled={!form.insuranceCoverage} {...(form.insuranceCoverage ? req('Enter insurance coverage price.') : {})}
                        />
                        {errors.insuranceCoveragePrice && <div className="text-red-500 text-xs mt-1">{errors.insuranceCoveragePrice}</div>}
                      </div>
                    </div>

                    {/* More Features (name + price) */}
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <label className="block text-[14px] font-medium text-gray-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                      <label className="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors duration-150">
                        <input type="checkbox" name="addDriver" checked={form.addDriver} onChange={handleChange} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" />
                        <span className="ml-2 text-sm text-gray-700">Add Driver</span>
                      </label>
                      <div className="md:col-span-2">
                        <input
                          type="number" min="0" step="0.01" name="addDriverPrice" value={form.addDriverPrice}
                          onChange={handleChange} placeholder="Driver price per day (0.00)" className={inputClasses('addDriverPrice')}
                          disabled={!form.addDriver} {...(form.addDriver ? req('Enter driver price.') : {})}
                        />
